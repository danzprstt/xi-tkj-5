...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // Data dashboard pengurus kelas (/private-pr-kelas) — MENULIS tertutup
    // total dari client, cuma serverless function (Firebase Admin SDK) yang
    // boleh tulis, dan itu pun sudah diverifikasi token sesi pengurus dulu
    // di dalam kodenya (lihat api/verify-officer-key.js, api/tugas.js,
    // api/kas.js, api/pengumuman.js). TAPI boleh dibaca publik — supaya
    // hasilnya (tugas/PR, kas, pengumuman) bisa ditampilkan ke semua siswa
    // di halaman utama, cuma EDIT-nya yang privat khusus pengurus.
    match /officerKeys/{role} {
      allow read, write: if false; // ini beda — hash kode pengurus, tetap tertutup total
    }
    match /tugasPR/{id} {
      allow read: if true;
      allow write: if false;
...
## 🗂️ Dashboard Pengurus Kelas (`/private-pr-kelas`)

Halaman **privat**, tidak ada link-nya di navigasi publik — cuma bisa diakses
kalau tahu URL-nya langsung. Dipakai 4 pengurus kelas (Ketua, Wakil Ketua,
Sekretaris, Bendahara) untuk:

- **Tugas & Deadline** — catat PR/tugas per mapel beserta deadline-nya
- **Dokumentasi Uang Kas** — catat pemasukan/pengeluaran kas kelas + saldo otomatis
- **Moderasi** — lihat & hapus komentar siswa yang lolos filter kata kasar

Semua data ini (`tugasPR`, `kasKelas`) **tidak pernah bisa dibaca langsung
dari Firestore client** — rules-nya menutup total, cuma serverless function
yang boleh akses (lihat bagian rules di atas). Jadi walau publik tahu URL
`/private-pr-kelas`, mereka tetap butuh kode akses salah satu dari 4 role itu
buat lihat isinya.

### Setup (sekali di awal)

Sama seperti setup kode edit siswa, tapi generate kode buat 4 pengurus:

```bash
# Pastikan service-account.json sudah ada di root (lihat bagian Fitur Edit Profil di atas)
node scripts/generate-officer-keys.js
```

Ini generate 4 kode (8 karakter, lebih panjang dari kode siswa karena aksesnya
lebih sensitif), dicetak ke terminal + disimpan ke `officer-keys-output.txt`.
**Simpan file itu baik-baik, lalu bagikan tiap kode ke pengurus yang
bersangkutan secara personal** (jangan di grup kelas).
