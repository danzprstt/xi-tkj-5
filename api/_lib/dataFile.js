...
/** Baca path `photo` yang SEDANG dipakai siswa no. `no`, sebelum diedit. */
export function getSiswaPhoto(fileText, no) {
  ...
  return getStringField(fileText.slice(braceStart, braceEnd + 1), 'photo');
}

/** Baca field `link` (path file lagu) yang SEDANG dipakai lagu nomor `no`. */
export function getLaguLink(fileText, no) {
  ...
  const braceEnd = findMatchingBrace(fileText, braceStart);
  if (braceStart === -1 || braceEnd === -1) return '';
  return getStringField(fileText.slice(braceStart, braceEnd + 1), 'link');
}
