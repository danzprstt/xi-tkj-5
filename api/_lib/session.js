...
/**
 * Wrapper khusus buat sesi pengurus kelas (ketua/wakil/sekretaris/bendahara).
 * Reuse mekanisme HMAC yang sama seperti sesi edit siswa, cuma "subject"-nya
 * dinamai `officer:<role>` supaya token ini TIDAK BISA dipakai kebalik buat
 * lolos verifikasi sesi edit siswa manapun (namespace terpisah).
 */
export function buatOfficerToken(role) {
  return buatSessionToken('officer:' + role);
}
export function verifyOfficerToken(token, role) {
  const { valid, sid } = verifySessionToken(token, 'officer:' + role);
  return { valid, role: valid ? role : null, sid };
}
