...
export default function MenfessSection() {
  const { menfessMap, likedSet, kirimMenfess, toggleLikeMenfess, pendingSet } = useMenfess();
        ...
        <div className="menfess-list">
          {items.length === 0 ? (
            ...
            items.map((m) => {
              ...
              const pending = pendingSet.has(m.id);
              return (
                <div className="menfess-item" key={m.id}>
                  ...
                  <button
                    className={'menfess-like-btn' + (liked ? ' liked' : '') + (pending ? ' pending' : '')}
