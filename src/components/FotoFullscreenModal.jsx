...
export default function FotoFullscreenModal({ mode, activeIndex, onClose, onOpenSingle, onNavigate }) {
  ...
  // PENTING: dirender lewat createPortal langsung ke document.body, BUKAN di
  // posisi asli komponen ini di pohon JSX (yang nested di dalam <section
  // id="foto">). Ini yang memperbaiki bug "modal tidak full-screen" — kalau
  // dirender di tempat aslinya, ancestor manapun di antaranya yang bikin
  // containing-block baru (transform/filter/will-change dari elemen .reveal,
  // .foto-track, dll di sekitarnya) bisa membuat position:fixed pada modal
  // ini "terjebak" mengikuti batas ancestor tsb, bukan viewport penuh.
  // Portal menghindari masalah itu sepenuhnya, sama seperti pola yang sudah
  // dipakai modal-modal siswa lain yang dirender di level atas App.jsx.
