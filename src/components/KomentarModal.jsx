...
function KomentarItem({ komentar, semua, isReply, likedSet, pendingSet, onLike, onReply, onHapus }) {
  ...
  const pending = pendingSet ? pendingSet.has(komentar.id) : false;
    ...
    <div className={'komentar-item' + (isReply ? ' komentar-reply' : '')}>
      ...
      <div className="komentar-content">
        ...
        <div className="komentar-actions">
          ...
          <button className={'komentar-like-btn' + (liked ? ' liked' : '') + (pending ? ' pending' : '')} onClick={() => onLike(komentar.id)}>
          ...
          <div className="komentar-replies">
            {replies.map((r) => (
              <KomentarItem
                ...
                likedSet={likedSet} pendingSet={pendingSet} onLike={onLike} onReply={onReply} onHapus={onHapus}
              />
            ))}
          </div>
        )}
...
export default function KomentarModal({
  ...
  likeCounts, likedSet, onToggleLike, likePendingSet, komentarPendingSet,
}) {
    ...
    >
      <div className="komentar-card">
        ...
        <div className="komentar-card-header">
          <div className="komentar-card-avatar"><ProtectedImage src={src} alt="" /></div>
          <div>
        ...
        <button className={'lightbox-like-btn' + (isLiked ? ' liked' : '') + (likePendingSet && likePendingSet.has(sid) ? ' pending' : '')} onClick={() => onToggleLike(sid)}>
