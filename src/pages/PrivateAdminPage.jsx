...
import {
  Lock, ArrowLeft, ClipboardList, Wallet, ShieldCheck,
...
function KasTab({ session }) {
  ...
  const [loading, setLoading] = useState(true);
  const [form, setForm] = useState({ id: null, tanggal: '', keterangan: '', tipe: 'masuk', jumlah: '' });
  const [busy, setBusy] = useState(false);
  const [status, setStatus] = useState('');
  ...
  const simpan = async () => {
    ...
    setBusy(false);
    if (res.ok) {
      setForm({ id: null, tanggal: '', keterangan: '', tipe: 'masuk', jumlah: '' });
      setStatus('✓ Tersimpan.');
      muat();
    } else setStatus('⚠ ' + (res.error || 'Gagal.'));
  };
          ...
          {list.map((t) => (
            <div className="private-admin-item" key={t.id}>
              <div>
                ...
                <div className="private-admin-item-sub">{t.tanggal}</div>
              </div>
              <div className="private-admin-item-actions">
                <button onClick={() => setForm({ id: t.id, tanggal: t.tanggal, keterangan: t.keterangan, tipe: t.tipe, jumlah: t.jumlah })}><Pencil size={13} /></button>
                <button onClick={() => hapus(t.id)}><Trash2 size={13} /></button>
              </div>
            </div>
          ))}
