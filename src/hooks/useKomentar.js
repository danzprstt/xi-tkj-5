...
export default function useKomentar() {
  ...
  // Sama seperti useSiswaLikes: kunci anti-dobel HARUS ref biasa yang diubah
  // langsung (synchronous), bukan derivasi dari React state, supaya tidak ada
  // celah waktu buat klik susulan nembus sebelum kunci ini aktif.
  const pendingRef = useRef(new Set());
  const likedSetRef = useRef(likedKomentarSet);
  likedSetRef.current = likedKomentarSet;
  ...
  const toggleLikeKomentar = useCallback((id) => {
    if (!id) return;
    ...
    // 1) Update tampilan instan (optimistic).
    toggleLikedLocal(id);
    setKomentarMap((prev) => {
    ...
    runTransaction(db, async (tx) => {
      ...
      .catch((e) => {
        console.error('Gagal like komentar, membatalkan optimistic update:', e);
        toggleLikedLocal(id);
        setKomentarMap((prev) => {
          if (!prev[id]) return prev;
          const current = prev[id].likes || 0;
  ...
  return { komentarMap, likedKomentarSet, kirimKomentar, toggleLikeKomentar, hapusKomentar, pendingSet: pendingUI };
}
